	...
	constructor(
		...
		this.addChild(
			new Text(
				theme.fg(
					"muted",
					`  ↑/↓: move. ←/→: page. ${branchKeys}: fold/branch. ${keyText("app.tree.editLabel")}: label. ${filterKeys}: filters (${cycleKeys} cycle). ^W/⇧^W: time. ${keyText("app.tree.toggleLabelTimestamp")}: label time`,
				),
				0,
				0,
			),
